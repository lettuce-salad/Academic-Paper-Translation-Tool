...
class PDFStructureParser:
    ...
    def _is_equation(self, block: Block) -> bool:
        ...
        学術論文の本文は定義文などで数学記号を多く含むため、
        記号の絶対数だけで判定すると本文を誤って数式扱いしてしまう。
        そこで「記号の割合」を主条件とし、英単語の量で本文/数式を分ける。
        """
        text = block.content
        if len(text) == 0:
            return False
        ...
        # 通常の英単語（4文字以上）= 本文らしさの指標
        # 数式中の変数名は freq, act など短いものが多いので4文字以上に限定
        ...
        # 長い本文（英単語が多い）は数式記号を含んでも本文
        # 一般的な散文は100字あたり15語程度。10語以上なら本文とみなす
        if word_count >= 10:
            return False
