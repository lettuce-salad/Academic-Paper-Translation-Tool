...
<head>
    ...
    def _generate_block(self, block: Block, page: Page) -> str:
        ...
        # 数式の場合はMathJax形式に（ただし英語本文が多い場合は通常テキスト扱い）
        is_real_equation = False
        if block.type == BlockType.EQUATION:
            word_count = len(re.findall(r'[A-Za-z]{4,}', content))
            if word_count < 8:
                is_real_equation = True
                if not content.startswith('$'):
                    content = f'$${content}$$'
        
        # HTMLエスケープ（MathJax処理する数式以外）
        if not is_real_equation:
